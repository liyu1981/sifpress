import { resolveVideo } from "./video-source";
import { cn } from "./cn";
...
export function buildVideoElement(
	options: BuildVideoElementOptions,
): HTMLElement | null {
	...
	if (video.kind === "file") {
		...
		v.src = video.src;
		v.title = options.alt;
		// The <video> element sizes itself from its intrinsic aspect ratio.
		v.className = cn(
		...
		iframe.loading = "lazy";
		// iframes have no intrinsic size; without an explicit aspect-ratio the
		// embed collapses to the browser's 150px default height (a flat bar).
		iframe.className = cn(
			"my-6 mx-auto block aspect-video w-full rounded-xl bg-black",
			options.className,
		);
		el = iframe;
	}
