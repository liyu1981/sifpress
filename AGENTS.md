...
- `build.php` runs `pnpm run build` in `admin_ui/`, inlines the built
  ...
  `spa.php`, `embed.php`, `migrations.php`, `dev.php`, `router.php`)
...
- **CLI**: `php sifpress.php [setup|migrate|change_password|inject_sifront|update_sifront|status|help]`
  ...
  when new; `--activate` makes it the active sifront. See `src/cli.php`.
...
src/                PHP source fragments (edit these)
  ...
  cli.php           CLI: setup / migrate / update_sifront / status
