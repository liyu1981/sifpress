...
src/                PHP source fragments (edit these)
  ...
  asset.php         ?module=asset binary blob serving (assets + thumbnails)
  ...
  favicon.php       ?module=favicon serving (icon + apple-touch-icon)
  ...
  dev.php           dev-only ?module=dev handler (dev builds only)
...
- **Migrations**: `migrations/*.sql` are embedded into `dist/index.php` at
  ...
  `POST ?module=migration&action=run` applies them (per-migration
