...
import { migrationApi } from '@/lib/pages'

/**
 * Full-screen maintenance view shown when the database schema is behind
 * the embedded migrations. Runs ?module=migration&action=run, then
      ...
      <div className="relative z-10 flex min-h-screen items-center justify-center px-6 py-12">
        <div className="glass-control w-full max-w-md rounded-2xl p-8">
          <div className="flex flex-col items-center gap-4 text-center">
            ...
            {run.isError && (
              <p className="text-sm text-destructive">{t('migration.error')}</p>
            )}
          </div>
        </div>
      </div>
