...
function makeBlobs(palette: Palette): Blob[] {
  return Array.from({ length: palette.count }, () => {
    const hue =
      HUES[Math.floor(Math.random() * HUES.length)] + (Math.random() * 24 - 12)
  ...
  useEffect(() => {
    ...
    const render = (t: number): void => {
      ...
      for (const blob of blobs) {
        const x = wrap(
          blob.rx * width +
            blob.ampX * Math.sin(t * blob.freq + blob.phase) +
            blob.vx * t,
          -200,
          width + 200,
        )
        ...
        const radius =
          blob.radius * base * (1 + 0.08 * Math.sin(t * 0.3 + blob.phase))
