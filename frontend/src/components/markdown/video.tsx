 ...
 *   ![alt](?module=asset&id=3&kind=video)          app asset (video)
 ...
 * `![Alt|center](clip.mp4)`).
 *
 * App-asset URLs (`?module=asset&id=N`) carry no extension, so the copied
...
const ASSET_URL_RE = /[?&]module=asset\b/i
...
export function resolveVideo(src: string): ResolvedVideo | null {
  ...
  if (YOUTUBE_HOST_RE.test(src)) {
    const id =
      YOUTUBE_ID_RE.exec(src)?.[1] ?? /[?&]v=([A-Za-z0-9_-]{11})/.exec(src)?.[1]
